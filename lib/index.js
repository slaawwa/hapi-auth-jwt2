...
internals.implementation = function(server, options) {
  ...
  const authenticate = async function(request, h) {
    ...
    let decoded;
    let tryPOST = request.auth.credentials == 'tryPost';

    if (!token) {
      if (!tryPOST && request.method === 'post') {
        request.auth.credentials = 'tryPost';
      ...
      if (h.unauthenticated) {
        ...
        request.auth.credentials = tokenType;
        if (request.route.settings.auth.mode === 'try') {
          return h.continue;
        } else {
          return raiseError('unauthorized', 'Invalid token', tokenType);
        }
      }
    }
    ...
    if (typeof options.validate === 'function') {
      ...
      for (let i = 0; i < keys.length; ++i) {
          ...
          if (i >= keys.length - 1) {
            ...
            if (h.unauthenticated) {
                ...
                if (request.route.settings.auth.mode === 'try') {
                    ...
                    return raiseError('unauthorized', err_message, tokenType);
                }
            }
          }
        ...
        try {
          ...
          if (request.auth.credentials === 'tryPost') {
